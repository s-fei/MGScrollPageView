...
@property (strong, nonatomic) MogoScrollView *scrollView;
...
-(void)setContentIndex:(NSInteger)contentIndex{
    if (_currentIndex != contentIndex) {
        [self setContentOffSet:CGPointMake(self.scrollView.bounds.size.width*contentIndex, 0) animated:true];
    }
}
...
//    self.currentView.frame = CGRectMake(self.currentIndex*self.bounds.size.width, 0, self.bounds.size.width, self.bounds.size.height);
//    self.oldView.frame =  CGRectMake(self.oldIndex*self.bounds.size.width, 0, self.bounds.size.width, self.bounds.size.height);
//    self.scrollView.contentSize = CGSizeMake(self.itemsCount*self.bounds.size.width, self.bounds.size.height);
...
//        childVc.view.frame =  CGRectMake(childVc.view.frame.origin.x, 0, self.bounds.size.width, self.bounds.size.height);
//    }];
//}
        ...
        if (page>=2) {// 需要滚动两页以上的时候, 跳过中间页的动画
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf) {
                    ...
                    [strongSelf didAppearWithIndex:currentIndex];
                    [strongSelf didDisappearWithIndex:_oldIndex];
                }
            });
        }
        else {
...
- (void)setupSubviewsWithCurrentIndex:(NSInteger)currentIndex oldIndex:(NSInteger)oldIndex scrollDirection:(ZJScrollPageControllerScrollDirection)scrollDirection {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (currentController == nil) {
            ...
            // 添加currentController
            if (currentController.zj_scrollViewController != self.parentViewController && currentController != nil) {
...
- (MogoScrollView *)scrollView {
    if (!_scrollView) {
        MogoScrollView *scrollView = [[MogoScrollView alloc] initWithFrame:self.bounds];
        
        self.currentView = [[UIView alloc] init];
        self.oldView = [[UIView alloc] init];
        scrollView.delegate = self;
        scrollView.excludeGestureClasss = self.segmentView.segmentStyle.excludeGestureClasss;
