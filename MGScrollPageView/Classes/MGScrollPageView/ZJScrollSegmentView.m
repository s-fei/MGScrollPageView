...
// 滚动条背景
@property (strong, nonatomic) UIView *scrollLineBgView;
// 遮盖
...
- (instancetype)initWithFrame:(CGRect )frame segmentStyle:(ZJSegmentStyle *)segmentStyle delegate:(id<ZJScrollPageViewDelegate>)delegate titles:(NSArray *)titles titleDidClick:(TitleBtnOnClickBlock)titleDidClick {
    if (self = [super initWithFrame:frame]) {
        ...
        _currentIndex = 0;
        _oldIndex = 0;
        _currentWidth = frame.size.width - segmentStyle.leftMargin - segmentStyle.rightMargin;
...
- (void)setupScrollViewAndExtraBtn {
    ...
    //        scrollW = maxX;
    //    }
    self.scrollView.frame = CGRectMake(self.segmentStyle.leftMargin, 0.0, scrollW, self.zj_height);
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        CGFloat lineX = (_currentIndex == 0 && _titles.count > 0 ) ? self.scrollLine.zj_x - self.segmentStyle.titleMargin : self.scrollLine.zj_x - self.segmentStyle.titleMargin/2;
        if (self.segmentStyle.lineWidth <= 0) {
            self.scrollLine.zj_x = lineX;
    ...
    if (self.scrollLineBgView) {
        self.scrollLineBgView.frame = CGRectMake(0, self.zj_height - self.segmentStyle.lineBgHeight, self.zj_width , self.segmentStyle.lineBgHeight);
    }
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            ...
            CGFloat width = (_currentIndex == 0 && _titles.count > 0 )? weakSelf.scrollLine.zj_width + weakSelf.segmentStyle.titleMargin*3/2:weakSelf.scrollLine.zj_width + weakSelf.segmentStyle.titleMargin ;
            CGFloat lineX = (_currentIndex == 0 && _titles.count > 0 ) ? weakSelf.scrollLine.zj_x - weakSelf.segmentStyle.titleMargin : weakSelf.scrollLine.zj_x - weakSelf.segmentStyle.titleMargin/2;
            if (weakSelf.segmentStyle.lineWidth <= 0) {
                weakSelf.scrollLine.zj_x = lineX;
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        CGFloat width = (currentIndex == 0 && _titles.count > 0 )? self.scrollLine.zj_width + self.segmentStyle.titleMargin*3/2:self.scrollLine.zj_width + self.segmentStyle.titleMargin ;
        CGFloat lineX = (currentIndex == 0 && _titles.count > 0 ) ? self.scrollLine.zj_x - self.segmentStyle.titleMargin : self.scrollLine.zj_x - self.segmentStyle.titleMargin/2;
        if (self.segmentStyle.lineWidth <= 0) {
            self.scrollLine.zj_x = lineX;
            self.scrollLine.zj_width = width;
        }
...
- (UIView *)scrollLineBgView {
    
    if (!self.segmentStyle.isShowLineBgView) {
        return nil;
    }
    
    if (!_scrollLineBgView) {
        UIView *lineView = [[UIView alloc] init];
        lineView.backgroundColor = self.segmentStyle.lineBgColor;
        
        _scrollLineBgView = lineView;
    }
    
    return _scrollLineBgView;
}
